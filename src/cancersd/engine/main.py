...
from cancersd.data.auxiliary import PatientDataset
...
if __name__ == '__main__':
    ...
    runtime_paths = prepare_runtime_paths(exp_cfg)
    for key, value in exp_cfg.items():
        print(f'{key}: {value}')

    print(runtime_paths.data_root)
    dataset_root = runtime_paths.data_root / exp_cfg['dataset']['root']
    print(dataset_root)
    print(runtime_paths.output_root)
    print(runtime_paths.log_root)

    omics_types = exp_cfg['dataset']['omics']
    subtype_number_dict = exp_cfg['dataset']['subtype_map']
    folder_suffix = None
    ...
    for opRound, seed in enumerate(seeds):
        ...
        # 基于整个测试集计算评估指标
        evaluation = get_performance_evaluation(
            all_targets,
            all_predictions,
            all_probabilities,
            'multiple'
    ...
    performance = {
        'accuracy': get_statistic(evaluations, 'accuracy'),
        'auc': get_statistic(evaluations, 'auc'),
        'recall': get_statistic(evaluations, 'recall'),
        'precision': get_statistic(evaluations, 'precision'),
        'f1_score': get_statistic(evaluations, 'F1 score'),
    }
