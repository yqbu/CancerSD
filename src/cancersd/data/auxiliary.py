...
class PatientDataset(BaseDataset):
    ...
    def __init__(self, omics_types, **kwargs):
        ...
        if kwargs.get('patients_info') is None:
            self.omics_dimensions, self.omics_dimensions_dict, self.patients_info = self.load_data(dataset_root,
                                                                                                   diagnosis_folder,
                                                                                                   kwargs)
    ...
    def load_data(self, root, diagnosis_folder, dicts):
        ...
        for moleType in self.omics_types:
            if os.path.exists(root / f'/patient_lack_{moleType}.npy'):
                patients_with_incomplete[moleType] = list(np.load(root / f'/patient_lack_{moleType}.npy'))
